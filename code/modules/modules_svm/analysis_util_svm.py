...
def create_metrics_dataframe(datasets, methods, results):
    ...
    highlight = lambda x: [
        (
            "border-left: 4px solid darkblue; border-right: 4px solid darkblue"
            if "weighted avg" in col
            else ""
        )
        for col in x.index
    ]
    ...
    # Display the tablea
    return (
        metrics_pivot.style.apply(highlight, axis=1)
        .set_table_styles(
            [
                {
                    "selector": "thead th",
        ...
        .set_caption("Performance Metrics for Each Method and Dataset")
    )
...
def show_class_occurrences(counts_df):
    ...
    display(
        counts_df.style.set_table_styles(
            [
                {
                    "selector": "thead th",
                    "props": [("background-color", "#f7f7f9"), ("color", "black")],
        ...
        .set_properties(**{"text-align": "center"})
        .set_caption("Table of Class Occurrences in Train, Test, and Predictions")
    )
...
def show_confusion_matrix(df, confusion_mtx):
    ...
    all_narratives = (
        df["narrative_subnarrative_pairs"]
...
def extract_confusion_data(df, target_label):
    ...
    for index, row in df.iterrows():
        ...
        true_indices = [label[1] for label in true_labels if label[1] == target_label]
        predicted_indices = [
            label[1] for label in predicted_labels if label[1] == target_label
        ]
...
def sort_narratives_recall(confusion_matrix, threshold_good=0.4, threshold_bad=0.1):
    ...
    recall_values = [
        (
            ...
            / (confusion_matrix[i][1][1] + confusion_matrix[i][1][0]),
        )
        for i in range(len(confusion_matrix))
        if confusion_matrix[i][1][1] + confusion_matrix[i][1][0] >= 5
    ]
...
def plot_difference_barchart(X, labels, sorted_narratives, label_mapping):
    ...
    plt.title(
        "Difference between (avg) narrative vector and (avg) vector over whole dataset."
    )
    plt.show()
...
def plot_differences_heatmap(X, labels, label_mapping, vectorizer, narratives=[]):
    ...
    plt.title(
        "Heatmap of 100 most important words for narrative(s): "
        + ", ".join(y_tick_labels)
    )
    plt.show()
