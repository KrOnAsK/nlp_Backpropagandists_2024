...
def prepare_data(df, model_name, output_dir):
    ...
    narratives = (
        ...
        .apply(lambda x: eval(x)[0] if isinstance(x, str) else x[0])
        .tolist()
    )

    unique_narratives = set(get_narrative_key(n) for n in narratives)
    label_mapping = {
        narrative: idx for idx, narrative in enumerate(sorted(unique_narratives))
    }
    ...
    train_texts = (
        df_train["tokens_normalized"]
        .apply(lambda x: " ".join(x) if isinstance(x, list) else x)
        .tolist()
    )
    val_texts = (
        df_val["tokens_normalized"]
        .apply(lambda x: " ".join(x) if isinstance(x, list) else x)
        .tolist()
    )
    ...
    train_encodings = tokenizer(
        train_texts, truncation=True, padding=True, max_length=512, return_tensors=None
    )
    val_encodings = tokenizer(
        val_texts, truncation=True, padding=True, max_length=512, return_tensors=None
    )
