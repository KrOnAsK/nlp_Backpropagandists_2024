...
def train_bert(
    ...
    try:
        ...
        # Initialize wandb
        wandb.init(
            project=project_name, name=f"bert-base-uncased-custom-{current_date}"
        )
        ...
        for narratives in all_narratives:
            if narratives:
                ...
                narrative_counts[narrative_str] = (
                    narrative_counts.get(narrative_str, 0) + 1
                )
        ...
        stratify_labels = df_filtered["narrative_subnarrative_pairs"].apply(
            lambda x: get_first_narrative_label(
                eval(x) if isinstance(x, str) else x, label_mapping
            )
        )

        df_train, df_val = train_test_split(
            df_filtered, test_size=0.2, random_state=42, stratify=stratify_labels
        )
