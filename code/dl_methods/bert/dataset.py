...
def get_first_narrative_label(
    narrative_list: List[Dict], label_mapping: Dict[str, int]
) -> Optional[int]:
    """
...
def prepare_data(df, label_mapping=None) -> Tuple[List[str], List[int], Dict[str, int]]:
    ...
    try:
        ...
        texts = [
            " ".join(tokens) if isinstance(tokens, list) else tokens for tokens in texts
        ]
        ...
        logger.error(
            f"Sample narrative_subnarrative_pairs: {df['narrative_subnarrative_pairs'].iloc[0]}"
        )
        raise
