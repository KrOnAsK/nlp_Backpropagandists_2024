...
def train_svm(
    ...
    emb_caching_path="code/models/multilingual_embeddings.npy",
):
    """
    ...
    try:
        ...
        if vectorizer == "tf-idf":
            ...
            if use_cached_embeddings and os.path.exists(
                os.path.join(base_path, emb_caching_path)
            ):
        ...
        flat_narratives = [
            item
            for sublist in df["narrative_subnarrative_pairs"].apply(eval)
            for item in sublist
        ]
        flat_labels = []
        ...
        counts_df = pd.DataFrame(
            {
                ...
                "Occurrences in Predictions": [
                    np.sum(y_pred[:, i]) for i in range(len(counts_original))
                ],
